...
soil_water = matrix(-6999,20,nrow(nameindex))
...
for (ii in 1:nrow(nameindex)){        
  ...
  input  = paste(nameindex$Year[ii],'/PV_site1-I-',nameindex$Year[ii],'-',
                 ...
                 '-g01.h5',sep='')
  soil_water[,ii] = h5read(input, "FMEAN_SOIL_WATER_PY")[,1]
  
}

#### Calculating daily values ####
soil_water.daily_average = rep(0,nfile/24)
day = 1

i = 1
while(i < ncol(soil_water)){
  soil_water.daily_average[day] = mean(soil_water[20,i:(i+23)]) #20 is top layer
  day = day + 1
  i = i + 24
}
...
#### Output data ####
write.csv(soil_water.daily_average, file = "DailySoilWater.csv")
